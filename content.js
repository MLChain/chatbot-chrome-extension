...
async function embedChatbot() {
  const mlchainChatbotConfig = window.mlchainChatbotConfig;
  if (!mlchainChatbotConfig) {
    console.warn('Mlchain Chatbot Url is empty or is not provided');
    return;
  }
  const openIcon = `<svg
            id="openIcon"
  ...
  function createIframe() {
    ...
    iframe.id = 'mlchain-chatbot-bubble-window'
    iframe.src = mlchainChatbotConfig.chatbotUrl
  ...
  const targetButton = document.getElementById("mlchain-chatbot-bubble-button");

  if (!targetButton) {
    // create button
    const containerDiv = document.createElement("div");
    containerDiv.id = 'mlchain-chatbot-bubble-button';
    ...
    containerDiv.addEventListener('click', function () {
      const targetIframe = document.getElementById('mlchain-chatbot-bubble-window');
      if (!targetIframe) {
        createIframe();
